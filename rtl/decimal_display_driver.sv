...
module decimal_display_driver (
  ...
  generate
    for (i = 0; i < 3; i = i + 1) begin : g_digit_pair
      // Fill this in
      //As the "inputs" to the generate loop,
      //we have blank_a and value_a
      //value_a is to be turned into a BCD.
      //If blank_a is true, the BCD is zero.
      //The BCD is then fed to a HEX

      logic [3:0] tens;
      logic [3:0] ones;
      //Input value_a[i]
      binary_to_bcd u_BCD_conv (
          .bin (value_a[i]),
          .tens(tens),
          .ones(ones)
      );
